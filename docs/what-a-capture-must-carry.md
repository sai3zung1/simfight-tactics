...
| Entries that were read and never landed    | #228        | `data/manifest.json` describes Magic Roll and Slightly Magic Roll at length — their reward table, the order their conditions are read in, Loaded Dice raising the lowest face — and neither entry exists in the file, along with Hugeify and `sizeIncrease`. The capture holds 176 where the client defines 181 | the entries themselves, again                                         | continuous |
...
| The property names a shipped build strips            | #206        | 401 blocks hand-written across `data/` for want of them                                                       |
