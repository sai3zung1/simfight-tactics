...
| The property names a shipped build strips            | #206        | **Nothing deserializes without them, and it gates the rest.** Seven asset families all fail with _package has unversioned properties but mapping file is missing_, and no `.usmap` exists in either install. Reading under the property layer is not a way round: a curve table serialises its row map after the property block, not instead of it. The file itself can be written rather than dumped — a JSON one is accepted and gets as far as naming the class and reading its mode — but a minimal one yields no rows, on 1089 curve tables, across Live, PBE and a pre-Live build, and across engine versions 5.6 to 5.9. Why is not established |
