...
| The property names a shipped build strips            | #206        | **Nothing deserializes without them.** Measured on the Live client: loading any package fails with _package has unversioned properties but mapping file is missing_, and no `.usmap` exists anywhere in either install. This is not the last reading the chain needs, it is the first — the 401 hand-written blocks are a consequence, not the whole cost |
