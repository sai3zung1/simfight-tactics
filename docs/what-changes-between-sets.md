...
| Effect naming    | rewrites whole   | Each set names its variables afresh, and the project adopts those names rather than normalising them — `docs/adr/0009-riot-naming-is-the-vocabulary.md`. `ParameterName` stays an open `string`, and a rotation rewrites whatever reads it |
