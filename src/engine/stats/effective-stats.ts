  ...
  for (const source of sources) {
    switch (source) {
      ...
      case "critDamage":
        ...
        break;
      // Range has no field in the resolved view, so it lands nowhere and adds
      // nothing — deliberate wherever it appears, not an unhandled case.
      case "range":
        break;
      default: {
...
// Scaled amounts read `base`, not the running result: passing `stats` here
// would let one modifier scale off another's output.
